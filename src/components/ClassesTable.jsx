  ...
  return (
    <div className="flex flex-col gap-3">
      {dummyData.map((data,id) => (
        <div key={id} className="flex justify-between items-center rounded-md p-4 shadow-md">
          <Link to={`/volunteer-classes/${data.id}`} className=" hover:cursor-pointer">{data.title}</Link>
          <div className="flex justify-evenly gap-4 min-w-60">
          <p >{data.students}</p>
            <p>{data.created}</p>
            <Popover>
              <PopoverTrigger>
              ...
              <PopoverContent align="end" a className="w-40 p-0">
                <div className="p-3 text-center hover:cursor-pointer">Copy Code</div>
                <div className="p-3 text-center hover:cursor-pointer text-red-500">Delete</div>
              </PopoverContent>
            </Popover>
          </div>
        </div>
